    ...
    return (
        <div className="px-4 md:px-10 lg:px-20 py-10 font-sans">
            ...
            <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4 mb-4">
                <select className="border border-gray-300 rounded-md px-3 py-2 text-sm md:text-base w-full md:w-auto" onChange={(e) => handleStateSelect(e.target.value)}>
                    <option value="" disabled selected>State</option> {/* Placeholder */}
                    {
                        states.map(state => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredDealers.map((dealer, index) => (
                    <div className="p-6 border rounded-lg shadow-sm bg-white text-left hover:bg-[#166434] hover:text-white cursor-pointer transition duration-300 ease-in-out" key={index}>
